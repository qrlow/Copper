...
function gradeRecoveryLabel(row) {
  const grade = Number(row.grade_pct);
  const recovery = Number(row.recovery_pct);
  if (row.asset === "Rest of World aggregate" || grade <= 0 || recovery <= 0) {
    return "n.a.<br><small>aggregate or not modelled</small>";
  }
